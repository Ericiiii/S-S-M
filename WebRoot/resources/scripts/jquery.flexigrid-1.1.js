...
(function ($) {
	...
	$.addFlex = function (t, p) {
		...
		p = $.extend({ //apply default properties
			...
			async: true, // data sending async from Ajax
			dataType: 'json', //type of data for AJAX, either xml or json
			errormsg: '连接数据错误.',
			singleSelect: true, //single select row.
			sortorder: 'desc',
			sortname: 'created',
			usepager: true,
			nowrap: true,
			...
			title: false,
			idProperty: 'sid',
			pageTotal: '显示{from}到{to}条-总共{total}条记录.',
			beforeSizeText: '&nbsp;每页',
			afterSizeText: "条",
			beforePageText: '第',
		...
		var g = {
			...
			rePosDrag: function () {
				...
				$('thead tr:first th:visible', this.hDiv).each(function () {
					...
					// liyong
					if (p.checkbox) {
	                	$('div:eq('+n+')',g.cDrag).css({'left':cdpos + 19 - cdcounter + 'px'}).show();
	                }else{
	                	// liyong 'left': (!(browser.mozilla) ? cdpos - cdcounter : cdpos) + 'px'
	                	$('div:eq(' + n + ')', g.cDrag).css({'left': cdpos - cdcounter + 'px'}).show();
	                }
					// end
					cdleft = cdpos;
					cdcounter++;
			...
			addData: function (data) { //parse data
				...
				this.loading = false;
				if (!data) {
					$('.pPageTotal', this.pDiv).html(p.errormsg);
				...
				if (p.total === 0) {
					...
					p.page = 1;
					this.buildpager();
					$('.pPageTotal', this.pDiv).html(p.nomsg);
                    if (p.onSuccess) p.onSuccess(this);
					return false;
				}
				...
				if (p.dataType == 'json') {
					$.each(data.rows, function (i, row) {
						...
						if (p.checkbox) {
							var bth = $('<th/>');
							var bthck = $('<input type="checkbox" value="' + $(tr).attr('id') + '"/>');
							var objtr = $(tr);
							bthck.click(function() {
								var state = this.checked;
								if (p.singleSelect) {
									$('tbody tr', g.bDiv).find(":checkbox").attr("checked", false);
			...
			buildpager: function () { //rebuild pager based on new properties
				...
				var stat = p.afterPageText;
				stat = stat.replace(/{pages}/, p.pages);
				...
				var total = p.pageTotal;
				total = total.replace(/{from}/, r1);
				total = total.replace(/{to}/, r2);
				total = total.replace(/{total}/, p.total);
				$('.pPageTotal', this.pDiv).html(total);
			},
			populate: function () { //get latest data
				...
				$('.pPageTotal', this.pDiv).html(p.procmsg);
			...
			addCellProp: function () {
				$('tbody tr td', g.bDiv).each(function () {
					...
					if (pth.render) {
						this.innerHTML = pth.render(this.innerHTML);
					}
		...
		if (p.usepager) {
			...
			$(g.bDiv).after(g.pDiv);
			var html = ' <div class="pGroup"> <div class="pFirst pButton"><span></span></div><div class="pPrev pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pcontrol">' + p.beforePageText + ' <input type="text" size="4" value="1" /><span class="pPageStat"> 1 </span></span></div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pNext pButton"><span></span></div><div class="pLast pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"> <div class="pReload pButton"><span></span></div> </div> <div class="btnseparator"></div> <div class="pGroup"><span class="pPageTotal"></span></div>';
			...
			if (p.useRp) {
				...
				$('.pDiv2', g.pDiv).prepend("<div class='pGroup'><span class='pPageSize'>"+ p.beforeSizeText +"<select name='pageSize'>" + opt + "</select>"+ p.afterSizeText +"</span></div> <div class='btnseparator'></div>");
		...
		if ($('th', g.hDiv).length) {
			...
			if (p.checkbox) {
         		$('tr', g.hDiv).each(
                 	function () {
                     var tth = $('<td/>');
                     var tthck = $('<input type="checkbox"/>');
                     if (!p.singleSelect) {
	                     tthck.click(function () {
	                         if (this.checked) {
