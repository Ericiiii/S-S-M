...
(function($){
	...
	_setting = {
		...
		async: {
			/* 注：异步加载树，需要sql中配置sid查询参数，初始化所有根节点. */
			enable: false,
		...
		asyncNode: function(setting, node, isSilent, callback) {
			...
			$.ajax({
				...
				success: function(msg) {
					...
					if(setting.async.root){
						newNodes = newNodes[setting.async.root] || newNodes;
					}
					if (node) {
						node.isAjaxing = null;
						node.zAsync = true;
	...
	$.zTree = {
		...
		init: function(obj, zSetting, zNodes) {
			...
			} else if (setting.async.url && setting.async.url !== '') {
				view.asyncNode(setting);
			}
			return zTreeTools;
		}
	};
	var zt = $.zTree, consts = zt.consts;
})(jQuery);
/*
