 ...
 * is copyright 2009 A Beautiful Site, LLC.
 * 
 *首先是最重要的方法 
 *
 ...
 * extraParams (Object):
 *
 *为后台(一般是服务端的脚本)提供更多的参数.和通常的作法一样是使用一个键值对对象.如果传过去的值是{ bar:4 },将会被autocompleter解析成my_autocomplete_backend.php?param=foo&bar=4 (假设当前用户输入了foo). Default: {}
 *
 * result(handler, row)
 *
 *为选中后触发的事件。item为选中的相。
 ...
 *例如，我们可以在选中后直接跳转页面：
 *
 *1 var data = [ {text:'Link A', url:'/page1'}, {text:'Link B', url: '/page2'} ];
 *2 $("...").autocomplete(data, {
 *3   formatItem: function(row) {
 *4     return row.text;
 *5   }
 *6 }).result(function(event, row) {
 *7   location.href = row.url;
 *8 });
 *9 
 */
;(function($) {
...
$.Autocompleter = function(input, options) {
	...
	$input.bind(($.browser.opera ? "keypress" : "keydown") + ".autocomplete", function(event) {
		...
		function findValueCallback(param, data) {
			var result;
			if( data && data.length ) {
				for (var i=0; i < data.length; i++) {
					if( data[i].result.toLowerCase() == param.toLowerCase() ) {
						result = data[i];
						break;
					}
				}
	...
	// param: the term entered
	// sValue: the first matching result
	function autoFill(param, sValue){
		// autofill in the complete box w/the first match as long as the user hasn't entered in more data
		// if the last user key pressed was backspace, don't autofill
		if( options.autoFill && (lastWord($input.val()).toLowerCase() == param.toLowerCase()) && lastKeyPressCode != KEY.BACKSPACE ) {
	...
	function receiveData(param, data) {
		if ( data && data.length && hasFocus ) {
			stopLoading();
			select.display(data, param);
			autoFill(param, data[0].value);
			select.show();
		} else {
			hideResultsNow();
		}
	...
	function parse(rows) {
		return $.map(rows, function(row) {
	    	return {
	  	      ...
	  	      result: options.formatResult && options.formatResult(row, row[options.field]) || row[options.field]
	  	     };
	    });
	};
	...
	return {
		...
		load: function(param) {
			...
			if (options.matchSubset) {
				for (var i = param.length - 1; i >= options.minChars; i--) {
					var c = data[param.substr(0, i)];
					if (c) {
						var csub = [];
						$.each(c, function(i, x) {
							if (matchSubset(x.value, param)) {
